...
async function generarPDFFactura(facturaData) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Calcular valores como en confirmacion.html
            // 1. Subtotal real (suma de todos los items)
            const subtotalCalculadoDesdeItems = facturaData.items.reduce((sum, item) => {
                return sum + (parseFloat(item.subtotal_item || 0));
            }, 0);
            
            // 2. Obtener valores de la BD
            // El subtotal en BD es el subtotal real (suma de items sin descuentos)
            const subtotalBD = parseFloat(facturaData.subtotal || 0);
            // Usar el subtotal de la BD si es válido, sino calcular desde items
            const subtotalReal = subtotalBD > 0 ? subtotalBD : subtotalCalculadoDesdeItems;
            
            const deliveryFee = 1500; // Costo de entrega estándar
            const descuentosRecompensas = parseFloat(facturaData.descuentos || 0); // Solo descuentos de recompensas
            const totalRaw = parseFloat(facturaData.total_facturado || 0);
            const impuestosBD = parseFloat(facturaData.impuestos || 0);
            
            // 3. Calcular subtotalAfterPromotions
            // Opción 1: Desde los impuestos guardados (más preciso si los impuestos fueron calculados correctamente)
            // Opción 2: Desde el total guardado (fallback)
            let subtotalAfterPromotions;
            if (impuestosBD > 0) {
                // Los impuestos se calculan sobre subtotalAfterPromotions: taxes = subtotalAfterPromotions * 0.05
                // Entonces: subtotalAfterPromotions = taxes / 0.05
                // Usar cálculo más preciso sin redondear hasta el final
                subtotalAfterPromotions = impuestosBD / 0.05;
            } else {
                // Fallback: calcular desde el total
                // total = subtotalAfterPromotions + deliveryFee + taxes - rewardDiscount
                // Donde: taxes = subtotalAfterPromotions * 0.05
                // Entonces: total = subtotalAfterPromotions * 1.05 + deliveryFee - rewardDiscount
                subtotalAfterPromotions = Math.max(0, (totalRaw - deliveryFee + descuentosRecompensas) / 1.05);
            }
            
            // 4. Calcular descuentos de promociones
            const descuentosPromociones = Math.max(0, subtotalReal - subtotalAfterPromotions);
            
            // 5. Usar los impuestos de la BD (ya calculados correctamente sobre subtotalAfterPromotions)
            // Si no están disponibles, recalcular sobre subtotalAfterPromotions
            const impuestos = impuestosBD > 0 
                ? Math.round(impuestosBD) 
                : Math.round(subtotalAfterPromotions * 0.05);
            
            // 6. Verificar que el cálculo sea consistente con el total guardado
            // El total debería ser: subtotalAfterPromotions + deliveryFee + taxes - rewardDiscount
            const totalCalculado = Math.max(0, subtotalAfterPromotions + deliveryFee + impuestos - descuentosRecompensas);
            ...
            const deliveryFeeFormatted = formatNumber(deliveryFee);
            const impuestosFormatted = formatNumber(impuestos);
            const descuentosRecompensasFormatted = formatNumber(descuentosRecompensas);
            ...
            if (descuentosPromociones > 0.01) {
                ...
                // doc.text('Subtotal después de promociones:', labelX, yPos, { align: 'right', width: labelWidth })
                //    .text(`CRC ${subtotalAfterPromotionsFormatted}`, valueX, yPos, { width: valueWidth, align: 'right' });
                // yPos += lineSpacing;
            }
            ...
            // Impuestos
            doc.text('Impuestos (5%):', labelX, yPos, { align: 'right', width: labelWidth })
               .text(`CRC ${impuestosFormatted}`, valueX, yPos, { width: valueWidth, align: 'right' });
            
            yPos += lineSpacing;
            
            // Descuentos de recompensas (si aplican)
            if (descuentosRecompensas > 0.01) {
